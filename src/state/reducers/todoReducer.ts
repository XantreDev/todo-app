    ...
    produce(state, (draft) => {
        switch (action.type) {
            ...
            case "todo/change-status":
                const indexChangeStatus = draft.findIndex(
                    (value) => value.id === action.payload
                );
                draft[indexChangeStatus].completed =
                    !draft[indexChangeStatus].completed;
                draft[indexChangeStatus].creating = false;
                return;

            case "todo/delete":
                const indexDelete = draft.findIndex(
                    (value) => value.id === action.payload
                );
            ...
            case "todo/change-title":
                const indexChangeTitle = draft.findIndex(
            ...
            case "todo/create-todo":
                const lastId = ~~Math.max(...draft.map((todo) => todo.id));
                draft.unshift({
                    completed: false,
                    id: lastId + 1,
